...
| **剑指 Offer 31. 栈的压入、弹出序列** | 遍历pushed，将元素逐个入栈，若当前栈顶元素等于popped当前所指元素，则弹出栈顶元素并popped指针进一。 | O(n) | 栈 |
| **剑指 Offer 40. 最小的k个数** | 一堆排序解法。构建1-n的数组，然后从倒数第一个非叶子节点从后向前遍历，down(i)以递归方式进行节点位置的调整，判断当前节点与孩子节点大小并交换(若交换则按交换的孩子位置继续down)。二快排模版解法。[_A_][__B__]，若k的个数<=A长度则只递归A（最小的k个值在A区间里），否则只递归B并更新k-=A区间长度（A区间的值都满足最小的k个值，同时有一部分k-A长度个值在B区间里）。| O(logn) | 堆排序 |
